...
const dropdown = document.querySelectorAll('.select__dropdown');

dropdown.forEach(function (el) {
	const dropdownBtn = el.querySelector('.select__dropdown-button');
	const dropdownList = el.querySelector('.select__dropdown-list');
	const dropdownItems = dropdownList.querySelectorAll('.select__dropdown-item');
	const dropdownInput = el.querySelector('.select__dropdown-input');

	// Клик по кнопке. Открыть/закрыть select
	dropdownBtn.addEventListener('click', function (e) {
		e.preventDefault();
		dropdownList.classList.toggle('select__dropdown-list--visible');
		this.classList.add('select__dropdown-button--active');
	});

	// Выбор элемента списка. Передать содержимое. Закрыть дропдаун
	dropdownItems.forEach(function (item) {
		item.addEventListener('click', function (e) {
			...
			changeValueInput(this.dataset.value);
			dropdownList.classList.remove('select__dropdown-list--visible');
		});
	});

	// Закрыть дропдаун по клику снаружи
	document.addEventListener('click', function (e) {
		if (e.target !== dropdownBtn) {
			dropdownBtn.classList.remove('select__dropdown-button--active');
			dropdownList.classList.remove('select__dropdown-list--visible');
		}
	});

	// Закрыть дропдаун с клавиатуры
	document.addEventListener('keydown', function (e) {
...
function changeValueInput(value) {
	...
	inputPhone.placeholder = phoneMask[value];
};
...
function form_validate_input(input) {
	let error = 0;
	let input_g_value = input.getAttribute('data-text');
...
function form_clean(form) {
	...
	for (let index = 0; index < inputs.length; index++) {
		...
		el.value = el.getAttribute('data-text');
	}
...
function select_actions(original, select) {
	...
	for (let index = 0; index < select_options.length; index++) {
		...
		const select_option_value = select_option.getAttribute('data-text');
		const select_option_text = select_option.innerHTML;

		if (select_type == 'input') {
			select_input.addEventListener('keyup', select_search);
		} else {
			if (select_option.getAttribute('data-text') == original.value && !original.hasAttribute('multiple')) {
	...
	if (select_options) {
		...
		for (let index = 0; index < select_options.length; index++) {
			...
			if (select_option_value != '') {
				const select_option_text = select_option.innerHTML;
				select_options_content = select_options_content + '<div data-text="' + select_option_value + '" class="select__option">' + select_option_text + '</div>';
			}
		}
...
//Placeholers
let inputs = document.querySelectorAll('input[data-text],textarea[data-text]');
inputs_init(inputs);
...
function inputs_init(inputs) {
	if (inputs.length > 0) {
		for (let index = 0; index < inputs.length; index++) {
			...
			const input_g_value = input.getAttribute('data-text');
